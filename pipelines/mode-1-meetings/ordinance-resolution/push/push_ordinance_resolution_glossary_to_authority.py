#!/usr/bin/env python
"""
Ordinance/Resolution PUSH Part 1 (Glossary Authority Loader)

Loads glossary entities from Ordinance/Resolution PARSE outputs and pushes them into:
  - cco.registry
  - cco.identities
  - cco.observations
...
Strict invariant:
  ...
  - No writes to m1_ordinance_resolution.documents
"""
...
STATE_FILE = OR_ROOT / "ordinance_resolution_glossary_push_state.json"
MANIFEST_FILE = OR_ROOT / "M1_ORDINANCE_RESOLUTION_GLOSSARY_PUSH_MANIFEST.jsonl"
...
SOURCE_GLOSSARY_SCHEMA_VERSION = "m1.ordinance_resolution.glossary.v1"
PUSH_SCHEMA_VERSION = "m1.ordinance_resolution.glossary_push.v1"
SOURCE_LANE = "ordinance_resolution_parse_glossary"
TARGET_SCHEMA = "cco"
...
def compact_observation_fact(entity: dict[str, Any], source_payload: dict[str, Any]) -> dict[str, Any]:
    meta = source_payload.get("ordinance_resolution_metadata")
    ...
    return {
        ...
        "source_ordinance_resolution_code": str(source_payload.get("ordinance_resolution_code") or ""),
        "source_document_type": str(meta.get("document_type") or ""),
        "source_document_number": str(meta.get("document_number") or ""),
...
def run_push(
    ...
    run_id = f"RUN-OR-GLOSSARY-PUSH-{datetime.now().strftime('%Y%m%dT%H%M%S')}"
    ...
    try:
        for parse_json in candidates:
            ...
            or_code = parse_json.parent.name
            try:
                ...
                failure_rows.append(
                    {
                        ...
                        "ordinance_resolution_code": or_code,
            ...
            prev = state_records.get(or_code, {})
            if (
                not force
            ...
            source_id = str(payload.get("ordinance_resolution_code") or or_code).strip()
    ...
    if not dry_run:
        run_manifest = run_dir / "ordinance_resolution_glossary_push_manifest.jsonl"
        with run_manifest.open("w", encoding="utf-8") as f:
            for row in run_rows:
                f.write(json.dumps(row, ensure_ascii=True) + "\n")

        if failure_rows:
            failure_out = run_dir / "ordinance_resolution_glossary_push_failures.jsonl"
    ...
    print("=" * 72)
    print("ORDINANCE/RESOLUTION GLOSSARY PUSH SUMMARY (PART 1)")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Push ordinance/resolution glossary entities into CCO authority tables."
    )
