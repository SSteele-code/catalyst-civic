#!/usr/bin/env python
"""
Public Hearing PUSH (DB Loader)

Loads normalized public hearing PARSE records from
`_Sources/M1-Meetings/Public_Hearings/_output` into `m1_public_hearing` tables.

Strict invariant:
  - DB load only for public-hearing tables
  - No glossary writes
"""
...
STATE_FILE = PUBLIC_HEARING_ROOT / "public_hearing_push_state.json"
MANIFEST_FILE = PUBLIC_HEARING_ROOT / "M1_PUBLIC_HEARING_PUSH_MANIFEST.jsonl"
...
PUSH_SCHEMA_VERSION = "m1.public_hearing.push.v1"
TARGET_SCHEMA = "m1_public_hearing"
...
def ensure_target_tables(cur: Any) -> None:
    cur.execute("SELECT to_regclass('m1_public_hearing.notices'), to_regclass('m1_public_hearing.excerpts')")
    notices_reg, excerpts_reg = cur.fetchone()
    if not notices_reg or not excerpts_reg:
        raise RuntimeError(
            "Target tables missing. Apply _Infra/DATABASE/init/015_public_hearing_schema.sql before PUSH."
        )


def derive_notice_row(
    payload: dict,
    notice_code: str,
    ...
    summary = payload.get("public_hearing_excerpt_summary")
    summary = summary if isinstance(summary, dict) else {}
    lineage = payload.get("lineage") if isinstance(payload.get("lineage"), dict) else {}
    source_lane = str(payload.get("source_lane") or "").strip()

    notice_id = str(pusher.get("notice_id") or payload.get("public_hearing_code") or notice_code).strip()
    source_id = str(
    ...
    notice_type = clean_text(pusher.get("notice_label")) or clean_text(summary.get("notice_label")) or "PUBLIC_NOTICE"
    confidence = to_float(pusher.get("max_confidence"))
    if confidence is None:
        confidence = to_float(summary.get("max_confidence"))
    excerpt_count = to_int(summary.get("excerpt_count")) or 0
    content_mode = clean_text(pusher.get("content_mode"))
    is_complete = bool(pusher.get("is_complete_notice_document"))
    ...
    metadata = {
        ...
        "lineage": lineage,
        "public_hearing_excerpt_summary": summary,
    ...
    return {
        ...
        "notice_confidence": confidence,
        "content_mode": content_mode,
        "is_complete_notice_document": is_complete,
...
def derive_excerpt_rows(payload: dict, notice_id: str) -> list[dict]:
    excerpts = payload.get("public_hearing_excerpts")
    ...
    for idx, ex in enumerate(excerpts, start=1):
        ...
        metadata = {
            "source_excerpt_payload": {
                ...
                "match_strength": signals.get("match_strength"),
                "match_pattern": signals.get("match_pattern"),
            }
        }
        out.append(
            {
                "excerpt_row_id": f"{notice_id}.{excerpt_id}",
                "notice_id": notice_id,
...
def upsert_notice(cur: Any, row: dict) -> None:
    cur.execute(
        """
        INSERT INTO m1_public_hearing.notices (
            notice_id, source_id, source_lane, jurisdiction, meeting_date, notice_type,
            notice_confidence, content_mode, is_complete_notice_document,
            linked_source_pdf_code, excerpt_count, metadata
        )
        VALUES (
            %(notice_id)s, %(source_id)s, %(source_lane)s, %(jurisdiction)s, %(meeting_date)s, %(notice_type)s,
            %(notice_confidence)s, %(content_mode)s, %(is_complete_notice_document)s,
            %(linked_source_pdf_code)s, %(excerpt_count)s, %(metadata)s::jsonb
        )
        ON CONFLICT (notice_id) DO UPDATE SET
            ...
            notice_confidence = EXCLUDED.notice_confidence,
            content_mode = EXCLUDED.content_mode,
            is_complete_notice_document = EXCLUDED.is_complete_notice_document,
...
def replace_excerpts(cur: Any, notice_id: str, excerpt_rows: list[dict]) -> None:
    cur.execute("DELETE FROM m1_public_hearing.excerpts WHERE notice_id = %s", (notice_id,))
    if not excerpt_rows:
        return
    for row in excerpt_rows:
        cur.execute(
            """
            INSERT INTO m1_public_hearing.excerpts (
                excerpt_row_id, notice_id, excerpt_id, ordinal, kind, start_line, end_line,
                content, content_sha256, signals, metadata
            )
            VALUES (
                %(excerpt_row_id)s, %(notice_id)s, %(excerpt_id)s, %(ordinal)s, %(kind)s, %(start_line)s, %(end_line)s,
                %(content)s, %(content_sha256)s, %(signals)s::jsonb, %(metadata)s::jsonb
            )
...
def run_push(
    ...
    try:
        for parse_json in candidates:
            ...
            notice_code = parse_json.parent.name
            try:
                ...
                failure_rows.append(
                    {
                        ...
                        "public_hearing_code": notice_code,
            ...
            notice_row = derive_notice_row(payload, notice_code, parse_json, source_parse_sha256)
            notice_id = notice_row["notice_id"]
            excerpt_rows = derive_excerpt_rows(payload, notice_id)

            prev = state_records.get(notice_id, {})
            if (
                not force
            ...
            try:
                ...
                upsert_notice(cur, notice_row)
                replace_excerpts(cur, notice_id, excerpt_rows)
                conn.commit()
                ...
                row = {
                    ...
                    "notice_id": notice_id,
                    "source_id": notice_row["source_id"],
                    "source_lane": notice_row["source_lane"],
                    "meeting_date": notice_row["meeting_date"],
                    "notice_type": notice_row["notice_type"],
                    "notice_confidence": notice_row["notice_confidence"],
                    "source_parse_json": str(parse_json),
    ...
    if not dry_run:
        run_manifest = run_dir / "public_hearing_push_manifest.jsonl"
        with run_manifest.open("w", encoding="utf-8") as f:
            for row in run_rows:
                f.write(json.dumps(row, ensure_ascii=True) + "\n")

        if failure_rows:
            failure_out = run_dir / "public_hearing_push_failures.jsonl"
    ...
    print("=" * 64)
    print("PUBLIC HEARING PUSH SUMMARY")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Push public-hearing parse output records into m1_public_hearing tables.")
    parser.add_argument("--limit", type=int, default=None, help="Push first N public-hearing records.")
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Only push records from this parse run id (default: latest run id from parse state).",
    )
    parser.add_argument(
        "--all-output",
        action="store_true",
